...
void PlayerSystem::Configure(ecs::EntityManager& entities, ecs::EventManager& events) {
    ...
    FillStatesNameMap(&state_name_to_player_state_id_);

    //! remove
    events.Emit<PlayerInitiatedEvent>(player_);
    events.Emit<SkinChangeRequest>(state_name_to_player_state_id_, static_cast<int>(PlayerCommand::IDLE), "./assets/sprites/orc_berserk.png", player_);
}
...
void PlayerSystem::Update(ecs::EntityManager& entities, ecs::EventManager& events, ecs::TimeDelta) {
    if(changed_state_ != static_cast<int>(PlayerCommand::INVALID)){
        // events.Emit<SpriteSheetStateChangedEvent>(changed_state_, *entities.GetEntitiesWithComponents<PlayerTag>().begin());
...
void PlayerSystem::Recieve(const PlayerTextRequestEvent& event) {
    ...
    if (event.cmd_[0] == kSpriteAdd) {
        ...
        event_manager_->Emit<SkinChangeRequest>(state_name_to_player_state_id_, static_cast<int>(PlayerCommand::IDLE), event.cmd_[1], player_);
